...
persistence. The class determines **where** the artifact is stored and
**how long** it may be kept.

| `retention_class` | Storage location        | Lifetime                                   |
| ----------------- | ----------------------- | ------------------------------------------ |
| `ephemeral`       | Local temp only         | Deleted at end of the current run/request. |
| `session`         | Local + optional S3     | Kept for the session; may sync to S3.      |
| `long_term`       | S3 (durable) + SQLite   | Linked to a memory; retained indefinitely. |
| `archive`         | S3 (durable)            | Operational backup/snapshot; ops-managed.  |
...
| `status`                      | TEXT     | `pending` \| `uploaded` \| `failed` \| `deleted`.                                               |
| `created_at`                  | DATETIME |                                                                                                 |
