...
| `MVP-JDG` | Judgment-backed behavioral baseline for MVP | `landed` | `bun run ci` + current docs review | `passing` | `src/judgment/*`, `src/queue/worker.ts`, `src/context/*`, ADR-0017, DEC-039 | Summary-output proposal is landed; provider-output extraction and `current_operating_view` remain planned/deferred. |
...
| `JDG` | DB-native Judgment System and memory-to-judgment convergence | `JDG-1C` | `in_progress` | Runtime foundation and summary-output proposal are landed; review visibility and provider-output extraction remain future. |
...
| `JDG-1C.2` | `MVP-JDG` | `JDG` | `JDG-1C` | Automatic Judgment extraction/proposal from provider or summary output | `JDG-1C.1` | New tests + docs update | `defined` | `in_progress` | ADR-0017; Q-027; `src/judgment/summary_proposals.ts` | Summary-output proposal is landed in `JDG-1C.2a`; continue with review visibility in `JDG-1C.2b` or provider-output boundary in `JDG-1C.2c`. |
...
| `P0-8A.2` | `P0-M3` | Phase 8 | Summary generation and memory summary persistence | `bun run ci` | `passing` | `landed` | `src/memory/summary.ts`; `test/memory/summary.test.ts`; AC-MEM-001, AC-MEM-002, AC-MEM-005, AC-MEM-006 | Summary-output Judgment proposal landed in `JDG-1C.2a`; review visibility is `JDG-1C.2b`. |
...
| `JDG-1B.4` | `MVP-JDG` | `JDG-1B` | Telegram write commands for propose/review/source/evidence/commit | `bun run ci` | `passing` | `landed` | `src/queue/worker.ts`; `test/queue/judgment_commands.test.ts` | Auto-proposal review visibility is `JDG-1C.2b`. |
...
| `JDG-1C.2b` | `MVP-JDG` | `JDG-1C` | Add review/operator visibility for auto-proposed summary judgments | New command/notification tests | `defined` | `ready` | Depends on `JDG-1C.2a`; existing `/judgment` commands | Decide whether summary notification includes proposal counts or separate query guidance. |
...
2. If the goal is feature development instead of acceptance work,
   the next ready feature leaf is `JDG-1C.2b`.
