...
|---|---|
| Test, lint, typecheck, build, eval commands | `docs/TESTING.md` |
| Runtime environment, deployment ownership, secrets, rollback notes | `docs/OPERATIONS.md` |
...
Actwyn currently targets `main` directly under the temporary policy documented
in `docs/policies/TEMP_MAIN_PUSH.md`. Treat `main` as the active integration and
release branch while that policy is in effect.

When the temporary policy ends, document the replacement branch flow here. The
default for a multi-branch service is `feat/*` or `fix/*` -> `dev` -> `main`,
where `dev` is the integration branch and `main` is the release source.
...
2. Put the real commands in `docs/TESTING.md`.
...
2. Identify the real source of truth for each command. Preserve exact command
   names and paths in `docs/TESTING.md`; do not normalize them into
...
4. Map existing environments to the environment inventory: preview, staging,
   ...
   external owner.
5. Record deployment ownership in `docs/OPERATIONS.md`: GitHub Actions,
