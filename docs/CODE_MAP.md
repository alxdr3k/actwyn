...
| `src/judgment/repository.ts`          | Proposal + proposal review + source recording + evidence linking + commit/activation transition: `proposeJudgment`, `approveProposedJudgment`, `rejectProposedJudgment`, `recordJudgmentSource`, `linkJudgmentEvidence`, `commitApprovedJudgment`. Each writes in a single `BEGIN IMMEDIATE` transaction. Approval does not activate. Source recording creates `judgment_sources` + `judgment_events` rows only. Evidence linking creates `judgment_evidence_links`, updates denormalized `source_ids_json`/`evidence_ids_json` on `judgment_items`, and appends `judgment.evidence.linked` — does not activate. Commit requires approved+evidence, sets `lifecycle_status=active` / `activation_state=eligible` / `authority_source=user_confirmed`, syncs denormalized arrays, appends `judgment.committed`. Active/eligible rows not read by runtime context. No supersede, revoke, expire, query, or explain path. | implemented (Phase 1A.2/1A.3/1A.4/1A.5 — not runtime-integrated)                     |
| `src/judgment/tool.ts`                | Unregistered local typed-tool contracts: `JUDGMENT_PROPOSE_TOOL`, `JUDGMENT_APPROVE_TOOL`, `JUDGMENT_REJECT_TOOL`, `JUDGMENT_RECORD_SOURCE_TOOL`, `JUDGMENT_LINK_EVIDENCE_TOOL`, `JUDGMENT_COMMIT_TOOL` plus the corresponding `execute*` functions. Wrap the repository and return stable `ok`/`error` results. **Not imported from any runtime module. Not registered anywhere.** | implemented (Phase 1A.2/1A.3/1A.4/1A.5 — not runtime-registered)                     |
