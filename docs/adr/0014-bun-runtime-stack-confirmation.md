...
현재 [`docs/TESTING.md`](../TESTING.md) 기준 자동화는 `bun test`,
...
> **Cleanup target (2026-04-27 follow-up).**
> [`src/queue/worker.ts`](../../src/queue/worker.ts) 의 `/doctor`
> command path 가 `Bun.version` 을 직접 fallback 으로 참조하고 있다.
> 본 ADR 의 P1 화이트리스트에는 worker 가 포함되지 않으므로, 이는
> 별도 PR (`refactor(queue): inject bun version into worker doctor
> path`) 에서 composition root ([`src/main.ts`](../../src/main.ts))
> 에서 주입하는 형태로 정리한다. 새 코드는 P1 을 그대로 따른다.
...
| Bun.S3Client 가 Hetzner 와 incompat                                | DEC-004 (Bun.S3Client with path-style; AWS SDK as P0.5 fallback) + `S3Transport` interface 유지. |
