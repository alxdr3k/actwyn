---
name: dev-cycle
description: "전체 개발 사이클: pull, doc 감사(codex), 구현, verify, review, ship. Flags: --loop [N], --phase id"
---

> **⛔ 이 스킬은 Codex 샌드박스에서 실행할 수 없습니다.**
> 이 스킬은 Claude Code CLI 오케스트레이터 전용입니다. Step 2의 `codex:rescue`,
> Step 5의 `/verify`, Step 6의 `/codex:adversarial-review` 등은 Claude Code 슬래시
> 커맨드이며 Codex 환경에서 실행되지 않습니다. Codex에서 이 스킬을 호출하면
> 검증·리뷰 게이트가 누락되어 검증되지 않은 변경이 main에 push될 수 있습니다.
> **Codex에서 이 스킬을 실행하지 마세요.** Claude Code 터미널에서만 실행하세요.
...
4. **5회 초과 시:** 미해결 지적 사항은 `gh issue create`로 GitHub issue를 생성한다 (제목: `[review] <한 줄 요약>`, 본문: 미해결 항목 목록).
   - **high/critical 찾기가 남아 있으면:** Step 8(Ship)로 진행하지 말고 사용자에게 명시 승인을 받아라. 승인 없이 push/merge하지 않는다.
   - **medium 이하만 남은 경우:** issue 생성 후 Step 7로 진행한다. 멈추지 않는다.
...
- `package.json` → `package.json`에 `ci` 스크립트가 있으면 `bun run ci` 우선 사용 (`lint:redactor + typecheck + bun test` 전체를 포함하므로 redaction 불변식과 타입 오류도 검사함). `ci` 스크립트가 없으면 `bun test` 사용.
